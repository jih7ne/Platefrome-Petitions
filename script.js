        ...
        //la fonction utilitaire pour les requetes xml
...
//charger les petitions
        ...
        function displayPetitions(oldPetitions) {
            ...
            container.innerHTML = petitions.map(function(p, index) {
                ...
                const canDelete = currentUser && p.Email === currentUser.email;
                ...
                return '<div class="petition-card" style="animation-delay: ' + (index * 0.05) + 's;">' +
                    ...
                    (canDelete ? '<button class="btn btn-secondary" onclick="deletePetition(' + p.IDP + ')">Supprimer</button>' : '') +
                    '</div>' +
                    '</div>' +
                    '</div>';
            }).join('');
